...
def solve_advanced_adding_doubling(column, irradiance, wvl):
    ...
    for k in range(column.nbr_lyr - 1, -1, -1):
        ...
        if aads.mth_azi == 0:                
            ...
            aads.thermal_c[:n, k] = (
                np.sum(s_layer_trans_up[:n, :column.model_inputs.nb_streams] 
                + s_layer_refl_down[:n, :column.model_inputs.nb_streams], axis=1)
                                     )
        ...
        if aads.solar_flag:
            ...
            # upward direction
            v0[:n, :n] = (
                aads.w[k] * aads.ff[:, :-1, k] * aads.cos_weight[np.newaxis, :]
            )
            # downward direction
            v0[n:, :n] = (
                aads.w[k] * aads.bb[:, :-1, k] * aads.cos_weight[np.newaxis, :]
            )
            ...
            source_up = solar1[:n]
            source_up -= (
                s_layer_refl_down @ solar1[n:] 
                + s_layer_trans_up @ (np.exp(-aads.t_od[k] / aads.cos_sun) * solar1[:n])
            )
            ...
            source_down -= (
                s_layer_trans_down @ solar1[n:] 
                + s_layer_refl_up @ (np.exp(-aads.t_od[k] / aads.cos_sun) * solar1[:n])
            )
            ...
            if abs(v0[n2 - 1, n2 - 1]) > 1e-4:
                source_down[-1] += ((np.exp(-aads.t_od[k] / aads.cos_sun) 
                                     - s_layer_trans_down[-1, -1]) 
                                    * sfac2 / v0[n2 - 1, n2 - 1])
            else:
                source_down[-1] -= (
                    np.exp(-aads.t_od[k] / aads.cos_sun) 
                    * sfac2 * aads.t_od[k] / aads.cos_angle[- 1]
                    )
        ...
        infinite_scattering = (identity_matrix 
                               - np.matmul(aads.s_level_refl_up[:, :, k + 1],
                                            s_layer_refl_down)
                               )
...
def solve_advanced_adding_doubling_fresnel(column, irradiance, wvl):
    ...
    for k in range(column.nbr_lyr - 1, -1, -1):
        ...
        if k == 0:
            ...
            denom1 = np.linalg.inv(identity_matrix 
                                    - s_layer_fresnel_refl_up @ s_layer_refl_down)
            denom2 = np.linalg.inv(identity_matrix 
                                    - s_layer_refl_down @ s_layer_fresnel_refl_up)
            
            
            # light traveling upwards: 
            s_layer_refl_up = (s_layer_refl_up 
                                + s_layer_trans_down 
                                @ denom1
            ...
            s_layer_refl_down = (s_layer_fresnel_refl_down 
                                + s_layer_fresnel_trans_up
                                @ denom2
                                @ s_layer_refl_down
        ...
        if aads.solar_flag:
            ...
            source_up -= (
                s_layer_refl_down @ solar1[n:] 
                + s_layer_trans_up @ (expfactor * solar1[:n])
            )
                
            source_down -= (
                s_layer_trans_down @ solar1[n:] 
                + s_layer_refl_up @ (expfactor * solar1[:n])
            )
            
            if abs(v0[n2 - 1, n2 - 1]) > 1e-4:
                source_down[-1] += ((expfactor 
                                     - s_layer_trans_down[-1, -1]) 
                                    * sfac2 / v0[n2 - 1, n2 - 1])
            else:
                source_down[-1] -= (
                    expfactor * sfac2 * aads.t_od[k] / aads.cos_angle[- 1]
                    )
