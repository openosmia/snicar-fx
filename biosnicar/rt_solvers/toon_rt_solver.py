...
His Matlab scripts were the jumping off point for this script.
...
def toon_solver(column, irradiance):
    ...
    """
    validate_inputs_toon(column, irradiance)
    # ----------------------------------------------------------------------------------
    ...
    g_star, ssa_star, tau_star = delta_transformation(column)
    ...
    tau_clm = calculate_optical_depth_of_column(column, tau_star)
    ...
    F_abs = absorbed_fluxes(column, F_net, F_top_net)
    ...
    outputs = get_outputs(
        column, irradiance, F_top_pls, F_top_net, F_btm_net, F_abs
    )
...
def validate_inputs_toon(column, irradiance):
    ...
    Args:
        column: instance of ColumnProperties
        irradiance: instance of SolarIrradiance
...
def delta_transformation(column):
    ...
    g = column.asm_prm 
    ss_alb = column.ss_alb
    tau = column.tau

    if column.modelconfig.inputs["RTM"]["DELTA"]:

        g_star = g / (1 + g)
        ssa_star = ((1 - (g**2)) * ss_alb) / (1 - (ss_alb * (g**2)))
        tau_star = (1 - (ss_alb * (g**2))) * tau
...
def calculate_optical_depth_of_column(column, tau_star):
    ...
    """
    tau_clm = np.zeros([column.nbr_lyr, column.nbr_wvl])

    for i in np.arange(1, column.nbr_lyr, 1):
...
def boundary_condition(column, irradiance, tau_clm, tau_star):
    ...
    s_sfc = (
        ...
        * irradiance.mu_not
        * np.exp(
            -(tau_clm[column.nbr_lyr - 1, :] + tau_star[column.nbr_lyr - 1, :])
            / irradiance.mu_not
        )
        * np.pi
        * irradiance.Fs
...
def two_stream_approximation(column, ssa_star, g_star, irradiance):
    ...
    if column.modelconfig.inputs["RTM"]["APRX_TYP"] == 1:
        # apply Eddington approximation
        gamma1 = (7 - (ssa_star * (4 + (3 * g_star)))) / 4
        gamma2 = -(1 - (ssa_star * (4 - (3 * g_star)))) / 4
        gamma3 = (2 - (3 * g_star * irradiance.mu_not)) / 4
        gamma4 = 1 - gamma3
        mu_one = 0.5

    elif column.modelconfig.inputs["RTM"]["APRX_TYP"] == 2:
        # apply quadrature approximation
        gamma1 = np.sqrt(3) * (2 - (ssa_star * (1 + g_star))) / 2
        gamma2 = ssa_star * np.sqrt(3) * (1 - g_star) / 2
        gamma3 = (1 - (np.sqrt(3) * g_star * irradiance.mu_not)) / 2
        gamma4 = 1 - gamma3
        mu_one = 1 / np.sqrt(3)

    elif column.modelconfig.inputs["RTM"]["APRX_TYP"] == 3:
...
def c_functions(
    ...
    """
    C_pls_btm = np.zeros([column.nbr_lyr, column.nbr_wvl])
    C_mns_btm = np.zeros_like(C_pls_btm)
    C_pls_top = np.zeros_like(C_pls_btm)
    C_mns_top = np.zeros_like(C_pls_btm)

    for i in np.arange(0, column.nbr_lyr, 1):

        if np.sum(irradiance.Fs) > 0.0:

            C_pls_btm[i, :] = (
                ssa_star[i, :]
                * np.pi
                * irradiance.Fs
                * np.exp(-(tau_clm[i, :] + tau_star[i, :]) / irradiance.mu_not)
                * (
                    ((gamma1[i, :] - (1 / irradiance.mu_not)) * gamma3[i, :])
                    + (gamma4[i, :] * gamma2[i, :])
                )
            ) / ((lam[i, :] ** 2) - (1 / (irradiance.mu_not**2)))
            ...
            C_mns_btm[i, :] = (
                ...
                * np.exp(-(tau_clm[i, :] + tau_star[i, :]) / irradiance.mu_not)
                * (
                    ((gamma1[i, :] + (1 / irradiance.mu_not)) * gamma4[i, :])
                    + (gamma2[i, :] * gamma3[i, :])
                )
            ) / ((lam[i, :] ** 2) - (1 / irradiance.mu_not**2))
            ...
            C_pls_top[i, :] = (
                ...
                * np.pi
                * irradiance.Fs
                * np.exp(-tau_clm[i, :] / irradiance.mu_not)
                * (
                    (gamma1[i, :] - (1 / irradiance.mu_not)) * gamma3[i, :]
                    + (gamma4[i, :] * gamma2[i, :])
                )
            ) / ((lam[1, :] ** 2) - (1 / irradiance.mu_not**2))
...
def matrix_solver(
    ...
    # Set up lists
    A = np.zeros([2 * column.nbr_lyr, column.nbr_wvl])
    B = np.zeros_like(A)
    D = np.zeros_like(A)
    E = np.zeros_like(A)

    for i in np.arange(0, 2 * column.nbr_lyr, 1):
        ...
        if i == 0:
            ...
            B[0, :] = e1[0, :]
            D[0, :] = -e2[0, :]
            E[0, :] = irradiance.Fd - C_mns_top[0, :]

        # BOTTOM LAYER
        elif i == 2 * column.nbr_lyr - 1:
            A[i, :] = e1[column.nbr_lyr - 1, :] - (column.sfc * e3[column.nbr_lyr - 1, :])
            B[i, :] = e2[column.nbr_lyr - 1, :] - (column.sfc * e4[column.nbr_lyr - 1, :])
            D[i, :] = 0.0
            E[i, :] = (
                s_sfc[:]
                - C_pls_btm[column.nbr_lyr - 1, :]
                + (column.sfc * C_mns_btm[column.nbr_lyr - 1, :])
            )
        ...
        # ODD NUMBERED LAYERS
        elif (i % 2 == 1) and (i < 2 * column.nbr_lyr - 1):
    ...
    AS[2 * column.nbr_lyr - 1, :] = np.nan_to_num(
        A[2 * column.nbr_lyr - 1, :] / B[2 * column.nbr_lyr - 1, :]
    )
    DS[2 * column.nbr_lyr - 1, :] = np.nan_to_num(
        E[2 * column.nbr_lyr - 1, :] / B[2 * column.nbr_lyr - 1, :]
    )
    ...
    for i in np.arange(2 * column.nbr_lyr - 2, -1, -1):
...
def layer_fluxes(
    ...
    """
    direct = np.zeros([column.nbr_lyr, column.nbr_wvl])
    F_net = np.zeros_like(direct)
    intensity = np.zeros_like(direct)
    F_up = np.zeros_like(direct)
    F_down = np.zeros_like(direct)
    F_btm_net = np.zeros([1, column.nbr_wvl])
    F_top_net = np.zeros_like(F_btm_net)
    F_top_pls = np.zeros_like(F_top_net)
    ...
    F_btm_net[0, :] = -F_net[column.nbr_lyr - 1, :]

    return F_net, F_top_pls, F_btm_net, F_top_net, intensity


def absorbed_fluxes(column, F_net, F_top_net):
    ...
    F_abs = np.zeros([column.nbr_lyr, column.nbr_wvl])
...
def get_outputs(
    ...
    """
    intensity_top = np.zeros(column.nbr_wvl)
    abs_vis = np.zeros(column.nbr_lyr)
    abs_nir = np.zeros(column.nbr_lyr)
    outputs = Outputs()
    ...
    heat_rt = outputs.abs_slr / (column.layer_mass * 2117)  # [K / s]
